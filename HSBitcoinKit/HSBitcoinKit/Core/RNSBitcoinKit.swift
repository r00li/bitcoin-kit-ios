...
//  HSBitcoinKit
//
//  Created by Andrej Rolih on 25/03/2019.
//  Copyright © 2019 Kamino. All rights reserved.
//
...
public class BitcoinKit {
    ...
    public init(withPublicKey: String, coin: Coin, walletId: String, newWallet: Bool = false, confirmationsThreshold: Int = 6, minLogLevel: Logger.Level = .verbose) {
        ...
        print("PUB KEY: \(try? hdWallet.publicKey(account: 0, index: 0, external: true))")
        
        if let key = try? hdWallet.publicKey(account: 0, index: 2, external: true) {
            print("KEY HASH: ", try? addressConverter.convert(keyHash: key.keyHash, type: .p2pkh).stringValue)
        }
